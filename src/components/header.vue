<template lang='pug'>
  .m-header.f-df
    .title.f-fs-20.f-fwb 登录服务
    .menu
      el-menu.f-ib.f-vam(:default-active="$route.path", mode="horizontal", router=true)
        el-menu-item.f-fs-16( index="/home") HOME
        el-menu-item.f-fs-16(v-show="$store.getters['user/isAdmin']" index="/manage/apps") APPS
        el-menu-item.f-fs-16(v-show="$store.getters['user/isAdmin']" index="/manage/users") USERS
    .userInfo.f-csp
